 ...
 *   node scan.mjs --no-jd-fetch                # do not fetch missing / full job descriptions (ATS APIs, Reed, Adzuna)
 *   node scan.mjs --no-browser-jd              # keep fetching, but never open Adzuna land links in headless Chromium
...
import { DESCRIPTIONS_PATH, appendDescriptions, enrichDescriptions, isRealDescription, readDescriptionLengths, shouldStoreDescription } from './job-descriptions.mjs';
...
// One title predicate for the scan loop: global title_filter + per-company
// overrides. Called as (title, entryName, jobCompany) — the override key is
// tried against BOTH the portals.yml entry name and the company the provider
// stamped on the job, because one board can carry several companies (Google's
// careers page lists DeepMind roles as company "DeepMind"). A config with no
// title_filter_overrides behaves exactly like buildTitleFilter(title_filter).
/**
 * `company_label` (optional per tracked_companies entry): the company name to
 * record on every job this entry returns, replacing whatever the provider put
 * there. Exists because some boards name the HIRING ENTITY rather than the
 * employer — amazon.jobs answers "AWS EMEA SARL (UK Branch)" / "Evi Technologies
 * Limited" and a reader searching the export for "Amazon" finds nothing. A
 * blank or non-string label means "not set". Mutates `jobs` in place; runs
 * BEFORE the blacklist and dedup so both see the label.
 * @param {Array<{company?: string}>} jobs
 * @param {{company_label?: unknown}} entry
 */
export function applyCompanyLabel(jobs, entry) {
  const label = typeof entry?.company_label === 'string' ? entry.company_label.trim() : '';
  if (!label || !Array.isArray(jobs)) return jobs;
  for (const job of jobs) {
    if (job && typeof job === 'object') job.company = label;
  }
  return jobs;
}

export function resolveTitleFilter(config) {
  const cfg = config && typeof config === 'object' ? config : {};
  const overrides = buildTitleFilterOverrides(cfg.title_filter_overrides);
  const fn = buildTitleFilterWithOverrides(cfg.title_filter, overrides);
  if (overrides.size === 0) return (title) => fn(title);
  const key = (v) => String(v ?? '').trim().toLowerCase();
  return (title, entryName, jobCompany) => {
    if (fn(title, key(entryName))) return true;
    const jc = key(jobCompany);
    return jc !== '' && jc !== key(entryName) && fn(title, jc);
  };
}
 ...
 * @param {string} [flag] - flag name to read (default '--since'); the same
 *   validation serves export-new-jobs.mjs's `--max-age-days`.
 * @param {{allowZero?: boolean}} [opts]
 * @returns {{days: number|null, error: string|null}}
 */
export function parseSinceDays(args, flag = '--since', { allowZero = false } = {}) {
  ...
  const occurrences = args.filter((a) => a === flag || a.startsWith(`${flag}=`));
  if (occurrences.length > 1) {
    return { days: null, error: `${flag} given ${occurrences.length} times; pass it once` };
  }
  if (occurrences.length === 0) return { days: null, error: null };
  const occ = occurrences[0];
  const next = args[args.indexOf(flag) + 1];
  const raw = occ.startsWith(`${flag}=`)
    ? occ.slice(flag.length + 1)
  ...
  // `allowZero` lets a caller accept 0 as "no window" (export-new-jobs.mjs
  // --max-age-days 0).
  if (!Number.isFinite(n) || n < 0 || (n === 0 && !allowZero)) {
    const want = allowZero ? 'a number of days (0 = no limit)' : 'a positive number of days';
    return { days: null, error: `${flag} expects ${want}, got ${raw == null || raw === '' ? '(no value)' : `"${raw}"`}` };
  }
  ...
  if (Number.isNaN(new Date(Date.now() - n * 86_400_000).getTime())) {
    return { days: null, error: `${flag} ${raw} is too large to express as a date` };
  }
...
const KNOWN_FLAGS = [
  ...
  '--since', '--quiet', '--json', '--no-jd-fetch', '--no-browser-jd', '--help', '-h',
];
...
async function main() {
  ...
  const fetchMissingDescriptions = !args.includes('--no-jd-fetch');
  const noBrowserJd = args.includes('--no-browser-jd');
  ...
  const titleFilter = resolveTitleFilter(config);
  ...
  // Round 3: URLs whose description text is already stored. Lets the loop
  // below record descriptions for already-known jobs it would otherwise just
  // dedup-skip — a free backfill on the first scan after the store appeared.
  const knownDescriptions = readDescriptionLengths(DESCRIPTIONS_PATH, normalizeUrlForDedup); // key → stored length
  // Round 3 audit aid: CAREER_OPS_TITLE_AUDIT=<file> writes every fetched title
  // with the filter's verdict, so the keyword lists can be checked against a
  // whole scan's worth of real titles (what was kept AND what was rejected).
  const titleAuditPath = process.env.CAREER_OPS_TITLE_AUDIT || '';
  const titleAudit = [];
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      applyCompanyLabel(jobs, company);
      ...
      for (const job of jobs) {
        ...
        const titleOk = titleFilter(job.title, company.name, job.company);
        if (titleAuditPath) {
          titleAudit.push([company.name, job.company || '', job.title || '', job.location || '', titleOk ? 'kept' : 'rejected']
        ...
        if (seenUrls.has(dedupUrl)) {
          totalDupes++;
          if (shouldStoreDescription(knownDescriptions.get(dedupUrl), job.description)) {
            knownDescriptions.set(dedupUrl, Infinity); // once per run
            descriptionBackfill.push({ url: job.url, description: job.description, source: sourceName });
          }
          continue;
        }
  ...
  if (titleAuditPath) {
    writeFileSync(titleAuditPath, 'entry\tcompany\ttitle\tlocation\tverdict\n' + titleAudit.join('\n') + '\n', 'utf-8');
    console.log(`Title audit: ${titleAudit.length} title(s) written to ${titleAuditPath}`);
  }
  ...
  // 5.6. Round 3: fetch the description of new offers that arrived without one
  // (Workday list payloads, Microsoft's synthetic one) from the ATS's public
  // JSON API, so the exports' Description column and the fingerprint below see
  // real text. Sequential, a few requests per run. Off for dry runs.
  // Round 6 (2026-09-22): Adzuna and Reed hand the scanner a ~500-char teaser
  // (`descriptionTruncated`); the same step fetches the whole posting for
  // those — Reed's per-job API, Adzuna's details page or the employer's page.
  let descriptionsFilled = 0;
  let descriptionsByVia = {};
  let descriptionsTeasers = 0;
  let descriptionsBrowserCapHit = 0;
  let descriptionsMissing = verifiedOffers.filter(o => !isRealDescription(o.description) || o.descriptionTruncated === true).length;
  if (!dryRun && fetchMissingDescriptions && descriptionsMissing > 0) {
    // `--no-browser-jd` keeps the cheap routes but never launches Chromium for Adzuna land links.
    const r = await enrichDescriptions(verifiedOffers, { browser: !noBrowserJd });
    descriptionsFilled = r.filled;
    descriptionsByVia = r.byVia;
    descriptionsTeasers = r.stillTruncated;
    descriptionsBrowserCapHit = r.browserCapHit || 0;
    descriptionsMissing = r.missing;
  }
  ...
  // Round 3: description store — new offers plus the backfill collected at the
  // dedup skip. Non-critical: a lock timeout here must never fail a scan whose
  // pipeline and ledger rows are already written.
  let descriptionsSaved = 0;
  let descriptionsBackfilled = 0;
  if (!dryRun) {
    try {
      descriptionsSaved = await appendDescriptions(DESCRIPTIONS_PATH, verifiedOffers);
      descriptionsBackfilled = await appendDescriptions(DESCRIPTIONS_PATH, descriptionBackfill);
    } catch (err) {
      console.error(`Warning: could not save job descriptions: ${err.message}`);
    }
  }
  ...
  if (!dryRun) {
    const viaList = Object.entries(descriptionsByVia).map(([via, n]) => `${via} ${n}`).join(', ');
    const viaApi = descriptionsFilled > 0 ? `, ${descriptionsFilled} full text fetched${viaList ? ` (${viaList})` : ''}` : '';
    const teasers = descriptionsTeasers > 0 ? `, ${descriptionsTeasers} teaser only (full text not reachable)` : '';
    const capHit = descriptionsBrowserCapHit > 0 ? `, ${descriptionsBrowserCapHit} over the browser cap — run: npm run backfill:descriptions -- --browser --source adzuna` : '';
    const unavailable = descriptionsMissing > 0 ? `, ${descriptionsMissing} unavailable` : '';
    console.log(`Descriptions saved:    ${descriptionsSaved} new, ${descriptionsBackfilled} backfilled${viaApi}${teasers}${capHit}${unavailable}`);
  }
  if (blacklist.size > 0) {
    if (includeBlacklisted) {
