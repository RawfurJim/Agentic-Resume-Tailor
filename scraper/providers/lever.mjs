...
/** Lever keeps the ISO 3166-1 alpha-2 country ("GB") in a top-level `country`
 *  field while `categories.location` is often a bare city ("Cambridge").
 *  scan.mjs judges location on one string, and a UK config cannot always_allow a
 *  bare "Cambridge" without admitting Cambridge, MA — so Healx's Cambridge UK
 *  roles were rejected (round 2, 2026-09-21). Append the code once, as
 *  "City, GB", the same "country code" form Google/Amazon locations use.
 *  Only a well-formed two-letter code is trusted; anything else is ignored.
 *  @param {string} location  @param {unknown} country */
function withCountry(location, country) {
  if (typeof country !== 'string' || !/^[A-Za-z]{2}$/.test(country.trim())) return location;
  const code = country.trim().toUpperCase();
  if (!location) return code;
  if (new RegExp(`(?<![a-z0-9])${code.toLowerCase()}(?![a-z0-9])`).test(location.toLowerCase())) return location;
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    return json.map(j => ({
      ...
      url: j.hostedUrl || '',
      company: entry.name,
      location: withCountry(resolveLocation(j.categories), j.country),
