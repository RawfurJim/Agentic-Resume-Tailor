...
// The search JSON already carries the full posting text, in three HTML fields.
// Joined into one plain-text description so scan.mjs can store it (there is no
// separate JD API for amazon.jobs, so without this the Description column and
// the CV matcher would have nothing). Returns undefined when all three are
// empty, so a missing description stays "unknown" rather than "".
function buildDescription(job) {
  const parts = [
    ['', job.description],
    ['Basic qualifications', job.basic_qualifications],
    ['Preferred qualifications', job.preferred_qualifications],
  ];
  const chunks = [];
  for (const [heading, html] of parts) {
